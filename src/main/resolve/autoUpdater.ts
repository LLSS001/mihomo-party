...
export async function checkUpdate(): Promise<IAppVersion | undefined> {
  ...
  const res = await axios.get(
    'https://github.com/xishang0128/clash-meta-party/releases/latest/download/latest.yml',
    {
...
export async function downloadAndInstallUpdate(version: string): Promise<void> {
  const { 'mixed-port': mixedPort = 7890 } = await getControledMihomoConfig()
  const baseUrl = `https://github.com/xishang0128/clash-meta-party/releases/download/${version}/`
  const fileMap = {
    'win32-x64': `clash-meta-party-windows-${version}-x64-setup.exe`,
    'win32-arm64': `clash-meta-party-windows-${version}-arm64-setup.exe`,
    'darwin-x64': `clash-meta-party-macos-${version}-x64.pkg`,
    'darwin-arm64': `clash-meta-party-macos-${version}-arm64.pkg`
  }
